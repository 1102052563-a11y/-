...
async function resolveBlueWorldInfoFileName() {
  const s = ensureSettings();
  const f = String(s.summaryBlueWorldInfoFile || '').trim();
  if (f) return normalizeWorldInfoFileName(f);
...
async function resolveGreenWorldInfoFileName() {
  ...
  if (t === 'file') {
    const nf = normalizeWorldInfoFileName(f);
    if (nf) return nf;
    // 自动命名（用户没填 file）：<chatbook>__SG_GREEN
    const chatbook = await getChatbookFileNameBySlashCompat();
